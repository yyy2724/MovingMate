...
$(document).ready(function () {
    ...
    function businessNumber() {
        ...
        var businessNumberEx = /^\d{3}-?\d{2}-?\d{3,5}$/;

        if (!businessNumberEx.test(busNum)){
            $('.businessNumber_short').css("display", "inline-block")
        }

        if (businessNumberEx.test(busNum)) {
            $('.businessNumber_expression').css("display", "none");
            $('.businessNumber_short').css("display", "none")
            $.ajax({
    ...
    function phoneCheck() {
        ...
        var validatePone = /^(01[016789]{1})[0-9]{3,4}[0-9]{4}$/
    ...
    // 이메일 발송
    function emailSend2() {
        alert("인증번호를 발송하고 있습니다. 잠시만 기다려 주세요.");
